...
| 8 | Footage Organizer | 8/12 | v2 LIVE — index + pull + --source flag on `feature/footage-organizer-v2`. SQLite index at `D:/Sai/.footage-index.sqlite`; `python cli_index.py --client sai pull --orientation vertical --filmed-date YYYY-MM-DD` builds Premiere-ready hardlink folders. v1 organize + archive flow unchanged. Flatten refactor (drop format folder, ORGANIZED → `<category>/<date>/`) Steps 0-3 partially shipped; Steps 4-8 pending. Rebase onto `308b4cd` was completed 2026-04-30 (was paused mid-flight, resolved by taking newer HEAD content for all conflicts). | Daily Sai loop: dump card → `main.py --client sai` → `cli_index.py --client sai index` → pull working sets. Finish flatten refactor: add `--allow-today` flag, write migration script, run on 7 dated folders, re-index, update workflow + README, real pull test, relocate 2026-04-27 Premiere project. Phase 2 (Whisper transcripts + semantic search) deferred. |
