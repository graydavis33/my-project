...
_Last updated: 2026-04-20 — Footage Organizer cache-sync guardrail shipped. `main.py` now runs `git fetch` + checks if remote has newer `.cache.json` before organize/archive/mark-used; warns + prompts to abort if behind. Tested live on Windows — caught a real Mac→Windows drift on commit `cd932e6` (Mac removed 5 stale cache entries). Prevents re-occurrence of the 2026-04-16 misc/ bug. April 16 misc/ folder cleanup still pending (40 clips in misc/ need manual re-sort, separate task)._
