...
Carry this forward every batch. Only update it when something actually changes. In Notion this is a single pinned page above the database.
...
- Editor: Section, Shot name, Shot type, Duration, Prop, Graphics / effect, Retention beat, Editor notes, Shot reference, Shot Notes.
...
Shot reference is a Files & media column: a reference photo / video / AI-generated illustration of how Gray wants this shot set up. Upload the file or paste a link in Notion on film-day prep. Mainly for the filmer (Gray) to match the setup; the editor can glance at it too. Empty by default — there's no markdown column for it; it's filled in Notion.

Fill the planning columns here; the on-set columns (Shot reference, Time of Day, Location, Shoot Day, Complete?) get set in Notion. The Editor brief above is the overview; the Editor notes column below is the clip-by-clip instruction — exactly what the editor does in that shot/duration.
