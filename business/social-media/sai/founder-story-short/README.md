...
- ✅ **WALL LOCKED 2026-08-07 evening: the wall beside the BATHROOM DOOR** (between the floor lamp and the door frame, dog bed underneath, where the rolling whiteboard parks now). Camera shoots across the room from the window side — the longest throw in the apartment — so the window sits BEHIND camera as soft frontal fill and never appears in frame. Gray's note "every board shot on Sai's right, facing the bathroom" survives. Full rationale + the two physical checks before hanging are in `REFERENCE-IMAGE-REVIEW-LOG.md` under WALL LOCK. **Open sub-item:** cork is now lit by cool daylight over the camera's shoulder while the Light columns call for 3200K — gel, blinds, or accept mixed. Call it on film day.
- ⚠️ **CORK BOARD DELIVERY vs FILM DAY.** Amazon reads Wed 08-12; the shoot is Tue 08-11. Gray's call 2026-08-07: **buy it in person this weekend** (Staples 1755 Broadway at 56th, or Target Columbus Circle). Ten of fifteen shots resolve on this board and there is no reshoot day. Confirm bought before Monday.
