...
**v3.2 — delivered watcher (`watch_delivered.py`, 2026-06-15):**
- Poll-based local watcher (no watchdog dep). `_delivered_videos` lists video files under 03_DELIVERED; `_stable_new_items` returns a new file only once its size is unchanged across two scans (export-done debounce); first run baselines existing files into `.ship-watch-state.json` so it reacts to NEW drops only.
- Reuses the ship engine headless: `_ship_plan` → post to Slack → `_await_reaction` polls `reactions.get` for ✅/❌ → `_execute_ship` + `_reindex`. Slack via slack_sdk WebClient + `SLACK_BOT_TOKEN`/`SLACK_USER_ID` (same pattern as morning-briefing/creator-intel); imported LAZILY so the pure helpers unit-test without slack_sdk or creds.
- NEEDS: `SLACK_BOT_TOKEN` + `SLACK_USER_ID` in .env + the bot's `reactions:read` scope (verify with `--self-test`). One watcher per machine (one-scheduler rule); Mac-only (local drive). launchd daemonization deferred until a clean foreground test.

**v4 — Structure guard (2026-06-21):**
- The 9 canonical top-level folders (`00_TEMPLATES` … `08_AI_EDITS`) sometimes get dragged INTO another in Explorer, or written nested by a tool whose root is set one level too deep (e.g. the batch-pipeline `.env` `SAI_LIBRARY_ROOT=D:/Sai/05_FOOTAGE_LIBRARY` → its `01_ORGANIZED`/`08_AI_EDITS` output lands at `05_FOOTAGE_LIBRARY/01_ORGANIZED`). This silently hides a top-level folder. Happened 2026-06-21: `01_ORGANIZED` (213 clips incl EP2 interviews) got nested in `05_FOOTAGE_LIBRARY`; recovered by moving it back.
- **`_find_misnested(library)`** detects any canonical folder nested one level inside another canonical folder. **`_repair_structure`** moves it back ONLY when the top-level slot is free (never overwrites/merges); warns otherwise. **`_guard_structure`** runs at the START of `index`/`pull`/`intake`/`ship`/`tag` (auto-heals), and **`check-structure [--repair]`** is the manual command. A **SessionStart hook** (`~/.claude/settings.json`, Windows) runs `check-structure --repair` every session (silent no-op when the drive isn't mounted).
- The fix is detection + auto-repair, not preventing the drag (can't). If a nested folder's top-level slot is occupied (both exist), it WARNS — merge by hand; do NOT auto-merge a folder another session is mid-writing (that's how the live batch `05_FOOTAGE_LIBRARY/08_AI_EDITS/shorts/_work` Vid-13 work was found 2026-06-21). Root-cause fix for the recurring nesting: correct the batch-pipeline root so it writes to `D:/Sai`, not `D:/Sai/05_FOOTAGE_LIBRARY`.
