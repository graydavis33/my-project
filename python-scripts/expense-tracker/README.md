...
- Skips expenses Gray already entered by hand in the app (`main.dedupe_vs_manual`, added 2026-07-08): gmail expenses dedup against this month's non-gmail Firestore records (`fetch_non_gmail_transactions` — manual entries AND Plaid rows, so Plaid suppresses duplicate email receipts); Plaid expenses dedup against truly-manual records only (`fetch_manual_only_transactions`). Match = same amount, dates within 2 days; tombstones suppress too. Local runs without Firestore keep everything.
