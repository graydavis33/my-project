...
def test_plan_drops_dead_air_and_maps_caption_times():
    ...
    keep, caps, total, synced_outs = cut.plan(words, ranges, audio, sr)
    assert len(keep) == 1
    assert caps[0]["word"] == "hello" and abs(caps[0]["start"]) < 0.2  # first kept word near t=0
    assert caps[1]["word"] == "world"
    assert total > 0.9
    # synced_outs: one entry per range, each at or after the range's sout (sin=10.0)
    assert len(synced_outs) == len(ranges)
    assert synced_outs[0] >= 10.0  # snapped out-point is at/after the range's sin
